...
function render() {
  ...
  const qualFilter = ($("#qualification")?.value || "").trim().toLowerCase();
  const items = state.items.filter((item) => {
    ...
    const passQual = !qualFilter || [
      item.qualification_requirement || "",
      ...(item.matched_keywords || [])
    ].join(" ").toLowerCase().includes(qualFilter);
...
["search", "region", "date-range", "source", "reg-date", "cutoff-date", "qualification"].forEach((id) => {
  $(`#${id}`).addEventListener(id === "search" || id === "reg-date" || id === "qualification" ? "input" : "change", render);
});
