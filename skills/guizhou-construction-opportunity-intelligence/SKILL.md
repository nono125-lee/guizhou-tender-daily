...
- `施工粗筛`：经过施工资质规则筛选的公告，并提供地区、时间、信息源、报名日期、截止日期和资质要求筛选。
- `招标计划`：展示全部招标计划的最新项目版本，并提供地州市、区县、发布时间、预计招标月和完整资金来源快捷筛选。
- `重点关联`：全部施工公告与超长期计划候选，显示相似说明和复核级别。
- `运行状态`：两个采集器的更新时间、数量、告警和本次运行结果。

统一页面标题固定为红色楷体“标讯雷达”。施工粗筛和招标计划栏目不显示复核状态筛选；复核状态只属于重点关联的本地处理动作。同一项目的多个计划版本在详细计划页保留历史入口。红色区域只承担近期行动清单，不铺开全部历史。
