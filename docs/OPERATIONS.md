...
6. 统一标讯雷达编排层：一次运行施工粗筛、招标计划、重点资金识别与关联、测试和发布，并生成单页工作台。
...
流程固定为：图文广告+园林绿化 → 施工粗筛 → 招标计划 → 国债/专项/中央/省级/超长期+用户重点关联 → 统一页面 → 三组测试 → 提交 `site/` → 推送 `main` → 更新 `gh-pages`。存在未提交的非 `site/` 代码改动时自动发布会停止，防止只发布数据而遗漏代码。
